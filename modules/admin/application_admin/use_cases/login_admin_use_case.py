...
class LoginAdminUseCase:
    ...
    def execute(self, email: str, password: str) -> common_value_objects.AuthenticationToken:
        """
        ...
        admin_user: AdminUserEntity | None = (
            self._admin_authentication_repository.find_admin_user_by_email_and_password(email=email, password=password)
        )

        if not admin_user:
            _LOGGER.error(f"Admin user with email [{email}] not found or invalid credentials")
            raise common_exceptions.UserNotFoundException(0) # Use a generic ID for admin login failure

        # Generate JWT token
        token_payload = {"user_id": admin_user.id}
        token = common_value_objects.AuthenticationToken.create_token(token_payload, self._secret_key)
