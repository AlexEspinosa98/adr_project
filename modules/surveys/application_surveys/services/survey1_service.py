...
class Survey1Service:
    def __init__(self, survey_repository: Survey1Repository, auth_repository: AuthRepository, user_producter_repository: UserProducterRepository, product_property_repository: ProductPropertyRepository, classification_user_repository: ClassificationUserRepository):
        self.logger = _LOGGER
    ...
    def create_survey1(self, data: CreateSurvey1InputDTO, producter_input_dto: SurveyUserProducterInputDTO, property_info_input_dto: PropertyInfoInputDTO, classification_user_input_dto: ClassificationUserInputDTO, api_key: str, image_paths: list[str]) -> CreateSurvey1OutputDTO:
        self.logger.info("Creating survey 1 with data: %s", data)
        
        use_case = CreateSurvey1UseCase(self.survey_repository, self.auth_repository, self.user_producter_repository, self.product_property_repository, self.classification_user_repository)
        survey_entity = use_case.execute(data, producter_input_dto, property_info_input_dto, classification_user_input_dto, api_key, image_paths)
