...
class Survey3Service:
    ...
    def create_survey3(self, data: CreateSurvey3InputDTO, producter_input_dto: SurveyUserProducterInputDTO, property_info_input_dto: PropertyInfoInputDTO, classification_user_input_dto: ClassificationUserInputDTO, api_key: str, image_paths: list[str]) -> CreateSurvey3OutputDTO:
        self.logger.info("Creating survey 3 with data: %s", data)
        
        use_case = CreateSurvey3UseCase(self.survey_repository, self.auth_repository, self.user_producter_repository, self.product_property_repository, self.classification_user_repository)
