...
from modules.surveys.application_surveys.use_cases.create_survey2_use_case import CreateSurvey2UseCase
...
class Survey2Service:
    ...
    def create_survey2(self, data: CreateSurvey2InputDTO, producter_input_dto: SurveyUserProducterInputDTO, property_info_input_dto: PropertyInfoInputDTO, api_key: str, image_paths: list[str]) -> CreateSurvey2OutputDTO:
        self.logger.info("Creating survey 2 with data: %s", data)
        
        use_case = CreateSurvey2UseCase(self.survey_repository, self.auth_repository, self.user_producter_repository, self.product_property_repository)
        survey_entity = use_case.execute(data, producter_input_dto, property_info_input_dto, api_key, image_paths)
