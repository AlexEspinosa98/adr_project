...
class PostgreSQLSurveyDetailRepository(SurveyDetailRepository):
    ...
    def get_survey_by_id_and_type(
        self, survey_id: int, survey_type: int
    ) -> Optional[Union[Survey1, Survey2, Survey3]]:
        if survey_type == 1:
    ...
    def get_classification_user_by_survey_id(
        self, survey_id: int, survey_type: int
    ) -> Optional[dict]:
        if survey_type == 1:
            stmt = select(ClassificationUser).where(
                ClassificationUser.survey_idd1 == survey_id
            )
            model = self.session.execute(stmt).scalar_one_or_none()
            return model.__dict__ if model else None
        elif survey_type == 3:
            stmt = select(ClassificationUser).where(
                ClassificationUser.survey_idd3 == survey_id
            )
            model = self.session.execute(stmt).scalar_one_or_none()
            return model.__dict__ if model else None
        else:
